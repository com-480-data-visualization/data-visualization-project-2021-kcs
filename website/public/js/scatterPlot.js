...
function update(x_axis, y_axis) {
  ...
  user_data.forEach(function (pl_data) {
    ...
    dots.enter().append("circle")
         .style("fill", function (d) { return color(pl_data.nb) } )
         .style("stroke", "white")
...
d3.select("#x-select-button").on("change", function(d) {
    // recover the option that has been chosen
    var x_axis = d3.select("#x-select-button").property("value");
    var y_axis = d3.select("#y-select-button").property("value");
...
// When the button is changed, run the updateChart function
d3.select("#y-select-button").on("change", function(d) {
    // recover the option that has been chosen
    var x_axis = d3.select("#x-select-button").property("value");
    var y_axis = d3.select("#y-select-button").property("value");
    // run the updateChart function with this selected option
